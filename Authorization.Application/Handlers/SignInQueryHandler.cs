...
public class SignInQueryHandler(TokenService tokenService,
    IAuthRepository authRepository) : IRequestHandler<SignInQuery, SignInDto>
{
    public async Task<SignInDto> Handle(SignInQuery request, CancellationToken cancellationToken)
    {
        var user = await authRepository.GetUser(request.Username);
